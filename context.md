...
### В работе
- [ ] Улучшение OCR для сканированных ТТН
  - **Проблема:** Tesseract PSM 11 + 600 DPI даёт мусор на табличных ТТН
  - **Решение (применено):** PSM 6 + 300 DPI + OEM 1 (LSTM)
  - Следующий шаг: тест на `1__ООО ПКФ ДИПОС ТТН УП-14562.pdf`

### Известные проблемы
- PDF-сканы с плотными таблицами (ТТН формат) плохо читаются через regex
  → рекомендуется режим Claude AI для таких документов
